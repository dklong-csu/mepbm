...
Models::FourStepAlternative::Parameters::Parameters()
    :
// just defer to the other constructor, using invalid values
    Parameters(std::numeric_limits<double>::signaling_NaN(),
               std::numeric_limits<double>::signaling_NaN(),
...
Models::FourStepAlternative::Parameters::Parameters(const double k_forward_value,
                                                     const double k_backward_value,
                                                     const double k1_value,
                                                     const double k2_value,
                                                     const double k3_value,
                                                     const double k4_value,
                                                     const double solvent_value,
                                                     const unsigned int nucleation_order,
                                                     const unsigned int maxsize_value,
                                                     const double particle_size_cutoff_value)
...
Models::FourStepAlternative::Parameters::operator std::valarray<double> () const
{
  return { k1, k2, k3, k4, k_forward, k_backward, particle_size_cutoff };
}



Models::FourStepAlternative::Parameters
Models::FourStepAlternative::Parameters::operator = (const Parameters &prm)
{
  k1 = prm.k1;
  k2 = prm.k2;
  k3 = prm.k3;
...
double Models::FourStepAlternative::rate_constant(const unsigned int size,
                                                   const Models::ParametersBase& parameters) const
{
  const Models::FourStepAlternative::Parameters& four_step_alt_parameters = dynamic_cast<const Models::FourStepAlternative::Parameters&>(parameters);
  if (size <= four_step_alt_parameters.particle_size_cutoff)
    return four_step_alt_parameters.k2;
  else
    return four_step_alt_parameters.k3;
}
...
unsigned int Models::FourStepAlternative::particleSizeToIndex(const unsigned int particleSize,
                                                               const ParametersBase& parameters) const
{
  const Models::FourStepAlternative::Parameters& fourStepAltParameters = dynamic_cast<const Models::FourStepAlternative::Parameters&>(parameters);

  if (particleSize == 1)
    return 0;
  else if ((particleSize >= fourStepAltParameters.w) && (particleSize <= fourStepAltParameters.maxsize))
    return particleSize - fourStepAltParameters.w + 3;
  else
    throw std::domain_error("Particle size outside of tracked range. Check the nucleation order and maximum particle size.");
}
...
unsigned int Models::FourStepAlternative::getSmallestParticleSize(const ParametersBase& parameters) const
{
  const Models::FourStepAlternative::Parameters& fourStepAltParameters = dynamic_cast<const Models::FourStepAlternative::Parameters&>(parameters);
  return fourStepAltParameters.w;
}



unsigned int Models::FourStepAlternative::getLargestParticleSize(const ParametersBase& parameters) const
{
  const Models::FourStepAlternative::Parameters& fourStepAltParameters = dynamic_cast<const Models::FourStepAlternative::Parameters&>(parameters);
  return fourStepAltParameters.maxsize;
}
...
double Models::FourStepAlternative::returnConcentration(const std::valarray<double>& particleSizeDistribution,
                                                         const unsigned int particleSize,
                                                         const ParametersBase& parameters) const
{
  const Models::FourStepAlternative::Parameters& fourStepAltParameters = dynamic_cast<const Models::FourStepAlternative::Parameters&>(parameters);

  return particleSizeDistribution[Models::FourStepAlternative::particleSizeToIndex(particleSize, fourStepAltParameters)];
}
...
std::valarray<double> Models::FourStepAlternative::right_hand_side(const std::valarray<double>& x,
                                                                    const Models::ParametersBase& parameters) const
{
  const Models::FourStepAlternative::Parameters& four_step_alt_parameters = dynamic_cast<const Models::FourStepAlternative::Parameters&>(parameters);
  ...
  f[4] = dn_{w+1}
  ...
  f[four_step_alt_parameters.n_variable - 1) = dn_{maxsize}
  */
  std::valarray<double> f(four_step_alt_parameters.n_variables);
  // precursor -- loss from dissociative step, gain from dissociative step
  f[0] = -four_step_alt_parameters.k_forward * x[0] * four_step_alt_parameters.solvent * four_step_alt_parameters.solvent + four_step_alt_parameters.k_backward * x[1]*x[2];
  // dissasociated precursor -- opposite effect from the dissociative step as the precursor
  f[1] = -f[0];
  // precursor -- loss from nucleation
  f[0] -= four_step_alt_parameters.k1 * x[0] * x[1] * x[1];
  // dissasociated precursor -- loss from nucleation
  f[1] -= 2 * four_step_alt_parameters.k1 * x[0] * x[1] * x[1];
  // skip ligand (p) for now since f[2] = - f[0] and we still need to update f[0]

  // nucleated particle -- gain from nucleation
  f[3] = four_step_alt_parameters.k1 * x[0] * x[1] * x[1];

  for (unsigned int i = 4; i < four_step_alt_parameters.n_variables; i++)
  {
    // particle gain from growth
    f[i] = rate_constant(four_step_alt_parameters.w + i - 4, four_step_alt_parameters) * x[0] * Models::available_atoms(four_step_alt_parameters.w + i - 4) * x[i - 1];
    f[0] -= f[i];
  }

  for (unsigned int i = 3; i < four_step_alt_parameters.n_variables - 1; i++)
  {
    // loss from growth
    f[i] -= f[i + 1];
  }
  ...
  // loss from growth on largest particle
  f[four_step_alt_parameters.n_variables - 1] -= Models::FourStepAlternative::rate_constant(four_step_alt_parameters.maxsize,four_step_alt_parameters) * x[0] * Models::available_atoms(four_step_alt_parameters.maxsize) * x[four_step_alt_parameters.n_variables - 1];
  f[0] -= Models::FourStepAlternative::rate_constant(four_step_alt_parameters.maxsize, four_step_alt_parameters) * x[0] * Models::available_atoms(four_step_alt_parameters.maxsize) * x[four_step_alt_parameters.n_variables - 1];
  // assign ligand rate now that precursor rate is final
  f[2] = -f[0];

  // pre-calculate contributions from each B particle
  std::valarray<double> agglomeration(0.0, four_step_alt_parameters.n_variables);
  for (unsigned int size = four_step_alt_parameters.w; size <= four_step_alt_parameters.particle_size_cutoff; ++size)
  {
    agglomeration[Models::FourStepAlternative::particleSizeToIndex(size, four_step_alt_parameters)]
      = Models::available_atoms(size)
        * x[ Models::FourStepAlternative::particleSizeToIndex(size, four_step_alt_parameters) ];
  }

  // loss from agglomeration
  for (unsigned int size = four_step_alt_parameters.w; size <= four_step_alt_parameters.particle_size_cutoff; ++size)
  {
    f[ Models::FourStepAlternative::particleSizeToIndex(size, four_step_alt_parameters) ]
      -= four_step_alt_parameters.k4
          * agglomeration[ Models::FourStepAlternative::particleSizeToIndex(size, four_step_alt_parameters) ]
          * agglomeration.sum();
  }

  // gain from agglomeration
  const unsigned int max_agglom_size = std::min( static_cast<unsigned int>(four_step_alt_parameters.particle_size_cutoff*2),
                                            four_step_alt_parameters.maxsize);
  for (unsigned int size = four_step_alt_parameters.w*2; size <= max_agglom_size; ++size)
  {
    for (unsigned int agglom_size = four_step_alt_parameters.w; agglom_size <= size/2; ++agglom_size)
    {
      f[ Models::FourStepAlternative::particleSizeToIndex( size, four_step_alt_parameters ) ]
        += four_step_alt_parameters.k4
            * agglomeration[ Models::FourStepAlternative::particleSizeToIndex( agglom_size, four_step_alt_parameters ) ]
            * agglomeration[ Models::FourStepAlternative::particleSizeToIndex(size - agglom_size, four_step_alt_parameters ) ];
    }
  }
