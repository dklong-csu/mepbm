...
Time2   = optimizableVariable('Time2',[0.1, 10],'Transform','none','Optimize',true);
Time3   = optimizableVariable('Time3',[0.1, 10],'Transform','none','Optimize',true);
...
stopFcn = @(results,state) stopBayesianOptimization(results,state,250,0.001);

max_cost_evals = 500;%50
% minimizer_filename = strcat(scenario,'.out');
acquisitionFcn = 'expected-improvement-plus';
results_filename = "opt_exp_4times_v2.mat";
