...
tic
fprintf("Running c++...")
exec = strcat("./convergence_check ",num2str(n_samps));
[status, cmdout] = system(exec);
% fprintf("%s",cmdout)
fprintf("done!!!!!! :-) ")
toc
%%
...
figure
plot(EMV)
ylabel('Average mean particle size')
xlabel('No. Monte Carlo Samples')
savefig('incremental_cost_EMV.fig')
close all

figure
plot(EPV)
ylabel('Average particle size variance')
xlabel('No. Monte Carlo Samples')
savefig('incremental_cost_EPV.fig')
close all

figure
plot(varMV)
ylabel('Mean particle size variance')
xlabel('No. Monte Carlo Samples')
savefig('incremental_cost_varMV.fig')
close all

figure
plot(varPV)
ylabel('Particle size variance variance')
xlabel('No. Monte Carlo Samples')
savefig('incremental_cost_varPV.fig')
close all