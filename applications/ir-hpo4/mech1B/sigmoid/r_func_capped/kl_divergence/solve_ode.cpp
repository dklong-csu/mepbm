...
int main (int argc, char** argv)
{
   ...
   */
  if (argc < 2){
    std::cout << "Usage: " << argv[0] << " <inpute filename>" << std::endl;
  ...
  MEPBM::IrHPO4::Mech1B<Vector, Matrix, Real, SampleType> mech(design);

  const std::vector<unsigned int> growth_height_indices = {3, 4, 5};
  const std::vector<unsigned int> growth_midpoint_sizes = {13, 55};
  const std::vector<unsigned int> growth_rate_indices = {6, 7};
  MEPBM::LogisticCurveGrowthKernel<Real, SampleType> growth_kernel(&MEPBM::r_func_capped<Real>,
                                                                   growth_height_indices,
                                                                   growth_midpoint_sizes,
                                                                   growth_rate_indices);

  const std::vector<unsigned int> agglom_height_indices = {8};
  const std::vector<unsigned int> agglom_midpoint_indices = {13};
  MEPBM::StepAgglomerationKernel<Real, SampleType> agglomeration_kernel(&MEPBM::r_func_capped<Real>,
