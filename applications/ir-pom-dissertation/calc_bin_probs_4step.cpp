...
create_ode(const Real kf, const Real kb, const Real k1, const Real k2, const Real k3, const Real k4, const unsigned int M)
{
...
int main (int argc, char** argv)
{
  ...
  #pragma omp parallel for
    ...
    */
    const std::string input_file = root_file + "-" + std::to_string(chain) + ".inp";
    const auto sample = MEPBM::import_parameters<SampleType>(input_file, 7);
    ...
    */
    ...
    auto rhs = [](Real t, N_Vector x, N_Vector x_dot, void* user_data){
      ...
      const auto k4 = prm[5];
      const auto M = prm[6];
      const auto rxns = create_ode(kf, kb, k1, k2, k3, k4, M);
    ...
    */
    ...
    for (unsigned int i=0; i<times.size(); ++i) {
      ...
      // Extract particles from solution vector
      auto particles = MEPBM::get_subset<Real>(solution.first, 3, 2500);

      // Generate the diameters so that binning can occur properly
      std::vector<Real> diams;
      for (unsigned int i=3; i<=2500; ++i){
        diams.push_back(MEPBM::atoms_to_diameter<Real>(i));
      }

      // Normalize the particle concentrations so that they are interpreted as probabilities
      ...
      const auto bin_probs = pmf.count;

      const std::string out = root_file + "-" + std::to_string(chain) + "-time" + std::to_string(i) + ".out";
      std::ofstream out_file;
      out_file.open(out);
      for (const auto p : bin_probs){
        out_file << p << "\n";
      }
      out_file.close();
    }
