...
int main (int argc, char** argv)
{
  ...
  // argv[3] = file containing counts in the beta distribution prior
  // argv[4] = file containing diameter for each of the counts in the beta prior

  const std::string root_file = argv[1];
  const int n_chains = std::atoi(argv[2]);
  const std::string beta_count_file = argv[3];
  const std::string beta_diam_file = argv[4];

  std::vector< std::vector<int> > beta_counts;
  for (unsigned int t=0; t<4;++t){
    beta_counts.push_back(
      MEPBM::import_parameters<std::vector<int>>(beta_count_file+"-"+std::to_string(t)+".txt", 27)
    );
  }
  const auto beta_diams  = MEPBM::import_parameters<std::vector<Real>>(beta_diam_file, 27);
    ...
    for (unsigned int i=0; i<times.size(); ++i) {
      ...
      if (log_likelihood > std::numeric_limits<Real>::lowest()) {
        ...
        if (solution.second != 0) {
          ...
          // add in the beta distribution counts
          std::vector<Real> tem_beta_data = tem_data[i];
          for (unsigned int bin=0; bin<beta_counts[i].size(); ++bin){
            for (unsigned int observation=0; observation<beta_counts[i][bin]; ++observation){
              tem_beta_data.push_back(beta_diams[bin]);
            }
          }
