%%
% Scenario 5
% Run 1 - Beta prior based on kde of data
% Run 2 - Beta prior based on average of kde of data and best sim
...
uq_link_root = "pom3step_scen5";
analysis_name = "Results_3step_scen5";
...
for tt=1:length(raw_data)
    prob_data = kde_to_bin_probability(raw_data{tt}, bin_edges);
    obs = draw_N_from_bins(prob_data, prior_obs);
    obs_file = strcat("betaObsRun1_",uq_link_root,"-",num2str(tt-1),".txt");
...
rng(seed,'twister')
% Likelihood function
betaCount_file = strcat("betaObsRun1_",uq_link_root);
...
for tt=1:length(raw_data)
    ...
    obs_file = strcat("betaObsRun2_",uq_link_root,"-",num2str(tt-1),".txt");
    obsID = fopen(obs_file,'w');
    fprintf(obsID,"%d\n",round(obs));
    fclose(obsID);
end
...
% Likelihood function
betaCount_file = strcat("betaObsRun2_",uq_link_root);
myLogLikelihood = @(params,y) custom_log_likelihood_times_beta(...
    ll_beta_exec_name, uq_link_root, betaCount_file, diams_file, params);
