...
if isempty(find(strcmp(varargin,'Kernel')))==0
    kern = varargin{find(strcmp(varargin,'Kernel'))+1};
end
%%
if size(fc,1)==1
...
for i=1:size(Y,2)
    ...
    if isempty(b)==0
        if isempty(supp)==0
            [f, u, bb]=ksdensity(Y{i},'bandwidth',b(i),'support',supp,'Kernel',kern);
        else
            [f, u, bb]=ksdensity(Y{i},'bandwidth',b(i),'Kernel',kern);
        end
    elseif isempty(b)
        if isempty(supp)==0
            [f, u, bb]=ksdensity(Y{i},'support',supp,'Kernel',kern);
        else
            [f, u, bb]=ksdensity(Y{i},'Kernel',kern);
        end
    end
