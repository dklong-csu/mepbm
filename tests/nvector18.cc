...
int main ()
{
  N_Vector num = MEPBM::create_eigen_nvector<Vector>(3);
  auto num_vec = static_cast<Vector*>(num->content);
  *num_vec << 1, 2, 4;


  N_Vector denom1 = MEPBM::create_eigen_nvector<Vector>(3);
  auto denom1_vec = static_cast<Vector*>(denom1->content);
  *denom1_vec << 4, 2, 1;


  N_Vector denom2 = MEPBM::create_eigen_nvector<Vector>(3);
