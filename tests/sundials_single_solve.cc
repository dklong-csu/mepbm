...
int main ()
{
  ...
  *ic_vec = Vector::Ones(1);
  ...
  // Solve the ODE
  auto solution = initial_condition->ops->nvclone(initial_condition);
  ode_solver.solve_ode(solution, end_time);

  // The answer should be close to exp(-10)
  auto sol_vec = static_cast<Vector*>(solution->content);
  std::cout << std::setprecision(20) << std::fixed << *sol_vec << std::endl;
}