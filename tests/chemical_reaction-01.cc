...
int main ()
{
  ...
  {
    MEPBM::ChemicalReaction<Real1, SparseMatrix1> rxn(reactants, products, rxn_rate);
    check_rxn_members(rxn);
  }

  {
    MEPBM::ChemicalReaction<Real1, DenseMatrix1> rxn(reactants, products, rxn_rate);
    check_rxn_members(rxn);
  }

  {
    MEPBM::ChemicalReaction<Real2, SparseMatrix2> rxn(reactants, products, rxn_rate);
    check_rxn_members(rxn);
  }

  {
    MEPBM::ChemicalReaction<Real2, DenseMatrix2> rxn(reactants, products, rxn_rate);
    check_rxn_members(rxn);
  }
}